...
- **SHIPPED (2026-09-20): Player page gets the same "Snapshot" card as Analytics' league-wide landing dashboard, scoped to that one player, placed ahead of everything else on the page.** Neil: "update each players page with the exact same table/format that is the first table on the analytics tab (except respectively filtered to each player) -- showing total bets placed, hit %, NFL and NCAA compare and then the % across the 4 categories. Add the most bet team as well. move this new table ahead of what was created yesterday."
  ...
  - Verified via Playwright: a 2-player mock (one player's picks only) confirms the card's bets-placed/accuracy/NFL-NCAA/category/most-bet-team numbers are scoped to that player alone and never leak the other player's pick (8/8 checks); re-ran the existing league-snapshot regression suite (14/14) and the By Matchup suite (12/12) to confirm the shared-function extraction didn't change either page's existing output.
  - Bumped service worker cache to `full-regalia-shell-v128`.

- **SHIPPED (2026-09-19): Player page's "By conference"/"By team" combined into one NFL/NCAA "By matchup" section with an Insights strip and conference→team drill-down; Ice Cold icon now shows its miss-streak count.** Neil: "as the season goes on the list of teams ... will be very long. Could we combine this with the above 'By Conference'?" — then asked for an aggregated summary and "insights" pushed to the top, plus a streak count on Ice Cold (Dumbass stays uncounted — it's a single-week award).
  1. **`reigningAwardWinners()`** (`js/awards.js`) now returns `iceCold` as a `Map<name, longestMissStreak>` instead of a bare `Set` — `awardIconsHtml()` (`js/season-data.js`) renders it as `🥶N` (mirroring the existing `🔥N` fire streak badge); `🤡` Dumbass is unchanged (no count — only ever one per week).
  2. **`ringMeterHtml()`/`recentFormHtml()`** moved out of `analytics.html`'s team-detail script into shared `js/pick-utils.js` (`recentFormHtml` regeneralized to take a plain ordered array of `"hit"/"miss"/"push"` instead of a team-game-log shape) so player.html could reuse them without duplicating the SVG-building code; `analytics.html`'s own call site just maps its game log to results first. `player.html` now also loads `js/team-stats.js` (needed for `computeTeamStreak`, which `recentFormHtml` calls).
  3. **player.html's "By matchup"** replaces the old flat "By conference" + capped/show-more "By team" cards with one NFL/NCAA chip toggle (same pattern as `analytics.html`'s sport toggle) driving: an **Insights strip** (most-bet team with a recent-form dot trail, best team bet on via a ring meter — min. 3-pick sample, most losses by team and by conference, "blind loyalty" = most-bet team with a losing record, "best underdog call" = the highest `+line` pick that hit) and a **conference → team drill-down** (tap a conference row to expand its teams, same `.team-history-toggle`/chevron pattern used elsewhere). Every insight is grounded in fields already on graded pick objects (`pick.team`, `pick.line`, `result`) — nothing new fetched.
  - Verified via Playwright: a 4-team/2-conference/12-pick scenario confirms every insight picks the right team/conference (most-bet, best-record, most-losses ×2, blind loyalty, best underdog call) and the conference drill-down expands/collapses correctly (12/12 checks); extended the existing award-icon test to assert `🥶2` renders with its count while `🤡` stays bare (12/12); full existing Team Trends/team-detail regression suite re-run clean after the `ringMeterHtml`/`recentFormHtml` extraction.
  - Bumped service worker cache to `full-regalia-shell-v127`.

- **SHIPPED (2026-09-19): Four requested updates — player page chart/history reformat, current Dumbass/Ice Cold icons in Standings, and Home standings polish.** Neil listed four separate asks in one message; planned all four together (confirmed via `AskUserQuestion` before building: stacked-bar chart over a cumulative line, and the new Regalia-Week history format scoped to player.html only, not the shared History tab).
  1. **Points-by-week chart** (`player.html`) is now a 2-color stacked bar grouped by **Regalia Week** instead of one bar per (sport, native week) — NFL and NCAA points for the same real week now show as two colors (`var(--accent)`/`var(--accent-2)`, a fixed categorical pair, never cycled) in one bar, with a small legend since it's now a 2-series chart. Total bar height still means "points that week," unchanged from before. Built following this session's `dataviz` skill (meter/bar mark specs, 2px surface gap between stacked segments, legend for 2+ series).
  2. **"By conference"/"By team" → weeks-list** on player.html now groups this player's picks by Regalia Week too, with NFL and NCAA sub-sections inside ONE card per week instead of two disconnected cards — a sport with no picks that week is skipped entirely (not a "no picks" placeholder). New `computeRegaliaWeekEntries()`/`renderRegaliaWeekEntry()` are local to player.html; the shared `computeHistoryEntries()`/`renderHistoryEntry()` (`js/season-data.js`) that `history.html` also depends on are untouched, per Neil's explicit scope confirmation.
  3. **Current Dumbass 🤡 / Ice Cold 🥶 icons** now show next to a player's name in Standings (both Home's preview and the full Standings page) — new `reigningAwardWinners(allAwards)` (`js/awards.js`) finds the most recent **FINAL** week's winners (using the existing `isRegaliaWeekFinal`) and returns them as name sets; `index.html` and `standings.html` (which now also loads `js/awards.js`) annotate each player object (`.isDumbass`/`.isIceCold`) the same way `.rank`/`.tied`/`.streak` already are, so `renderStandingsRow`/`renderStandingsRowCompact` just read the flag. Only rotates once a week is actually final — a still-in-progress current week never changes who's shown.
  4. **Home standings polish**: card title "Standings" → "Season Standings", column header "PTS" → "Points" (verified both still fit the narrow half-width card without wrapping); the 🔥 streak badge is now visibly smaller (`.standings-row-split .streak-badge`, 11px → 8.5px) than the Points number next to it, since the emoji glyph was reading as big as or bigger than the actual data.
  - Verified via Playwright: a 2-Regalia-Week/2-sport scenario for the chart+history reformat (10/10 checks, including confirming a sport with no picks is correctly skipped for that week); a real-final-week scenario with a tie for both awards, checked on both Home and Standings (12/12 checks, plus confirming title/column text and the streak-badge size fix); full existing player.html/Big-Dawg/Home regression suites re-run clean.
  - Bumped service worker cache to `full-regalia-shell-v126`.

- **SHIPPED (2026-09-16): Week-by-week's aggregate is now a donut/ring graphic, plus a full spacing cleanup across the team card.** Neil: "could there be like a donut graphic? ensure spacing looks clean between sections." Loaded this session's `dataviz` skill before building it.
  - Per the skill's own guidance (a single ratio is a **meter**, not a rainbow pie), built `ringMeterHtml(pct, label)` — an SVG ring where the fill carries the value (same magnitude color-mix already used everywhere else a percentage renders in this app — compare-pct, player-kpi-pct — not a new competing color rule) and the track is a calm neutral step, with the % direct-labeled in the center so the value is never color-only. Two of these (`atsGlanceHtml`) replace the plain "ATS 71% · 4O-6U" text that used to sit on the Week-by-week toggle, positioned above it so the answer is visible without expanding anything.
  - Separately, rebuilt the spacing inside `teamBlockHtml()`: every logical group (Recent Form, points tiles, glance+log, Minus/Plus/O-U, Home/Away, Who's picked) is now wrapped in one consistent `margin-bottom:14px` container instead of each piece carrying its own ad hoc margin — removed several now-redundant/conflicting inline margins in the process. This also fixed a real (if subtle) bug: `.stat-row`'s CSS `:last-child` rule was zeroing margin on whichever `.stat-row` happened to fall last across the ENTIRE flat sibling list (Minus/Plus/O-U AND Home/Away all mixed together), not the last row of each logical group — wrapping each group scopes `:last-child` correctly.
  - Verified via Playwright: full existing team-detail regression suite (single-team view, matchup view, history collapse, team search) all still passing; donut values spot-checked against the same 3-game cover-streak scenario used for Recent Form.
  - Bumped service worker cache to `full-regalia-shell-v125`.
...
- **SHIPPED (2026-09-16): Home/Away icons swapped from emoji to the site's own stroke-icon style; Week-by-week promoted above the season-aggregate rows.** Neil: "lets get rid of the emoji for home away and make them site format icons -- but i think the week by week info is more important than the minus, plus spread and the over/under." Both quick follow-ups to the same-day Team Trends additions.
  ...
  - Verified via Playwright (same 13-check suite, now also confirming the new section order in the rendered text).
  - Bumped service worker cache to `full-regalia-shell-v122`.

- **SHIPPED (2026-09-16): Team Trends gets a week-by-week game log and a home/away ATS split, keeping the existing points-summary.** Follow-up to Neil's "how else can we improve the analytics tab" — confirmed scope beforehand: week-by-week log + home/away split, explicitly keeping Points Scored/Allowed as-is (strength of schedule deferred to a later pass).
  - Refactored `js/team-stats.js`: `computeTeamRecord()` now derives from two new shared building blocks — `computeTeamGameLog(games, abbr)` (every finished game for a team, each enriched with its own cover result and O/U result, chronological) and `aggregateTeamGameLog(entries)` (rolls a list of those into the existing season-total record shape). This means the season aggregate, the new home/away split, and the new game log all read from the exact same per-game data instead of three separate hand-rolled loops that could drift apart.
  - New in `analytics.html`: an "ATS: Home vs Away" mini-section (Minus+Plus Spread combined into one overall cover % per location — splitting by both location AND category at once would've been 8 rows, more than needed) using the same `statRowHtml` bar component already used elsewhere; and a "Week-by-week" game log (opponent, final score, line, O/U, and a Hit/Miss/Push badge per game) — collapsed by default (same tap-to-expand pattern and CSS as the existing "Who's picked X" list) since a team deep into the season could have a lot of games.
  - Verified via Playwright (13/13 new checks on a 3-game mocked scenario with a home/away mix; existing team-detail regression suite — history collapse, no-odds handling, matchup view, team search — all still passing) and against real production data (a real team's ATS grading spot-checked by hand against the real final scores and lines).
  - Bumped service worker cache to `full-regalia-shell-v121`.

- **SHIPPED (2026-09-15): Follow-up fix — the week-based rewrite (previous entry, same day) had its own real bug: CFB's week query silently returns a curated ~25-game subset, not the full slate.** Neil noticed within hours: "still pending picks from completed games in week 2? and week 1... looks like a whole lot of week 1 and week 2 picks still pending." Confirmed directly: real CFB Week 1 has 99 games; the plain `week=1&seasontype=2&year=2026` query only returns 25 of them (no pagination info in the response — it's just a curated default subset), while adding `&groups=80` (ESPN's FBS group id) returns the real full 99. NFL has no such grouping concept and was unaffected. This meant a large number of real, already-finished picks were silently ungraded ("still pending" forever) because their specific games simply weren't in the fetched list at all — confirmed by name-checking the exact missing game IDs from real production picks data before and after the fix.
  - Fix: CFB requests in `fetchScoreboard()` now include `&groups=80`. Total games fetched roughly doubled (647 → 1282 across both sports) and every previously-missing real completed game (spot-checked several exact game IDs from real picks) is now present.
  - Verified via Playwright regression suite (unaffected — this only changes which real games are fetched, not the app logic) and directly against real production Supabase picks data: pending count dropped from 91 to 48, and every one of the remaining 48 is either genuinely future (Week 3, not yet played) or pre-existing known test/garbage rows (`TEST`, `TESTDEL`, fake game IDs like `9990001`/`nfl-g1`) that were never real games to begin with.
  - Bumped service worker cache to `full-regalia-shell-v120`.

- **SHIPPED (2026-09-15): Critical fix — ESPN broke the date-range query the entire site depended on; rebuilt game fetching around week-based queries instead.** What started as "why does a team's analytics page say 'no data' after 2 games" turned into a live, site-wide outage: confirmed directly against ESPN that its scoreboard endpoint now rejects ANY `dates=START-END` range query — even a 2-day range — with `400: Failed to get events endpoint`, for both NFL and CFB. This was the single request every page's game data flowed through (`fetchScoreboard()`/`loadSeasonGames()`, `js/live-scores.js` + `js/season-data.js`), and it fails silently (empty array, no visible error) — so Home, Standings, Picks, Awards, and Analytics were all likely showing blank/zero/"no data" for anyone loading a fresh page, not just NCAA team detail. Confirmed this broke sometime between Monday night (~10 PM ET, still working) and Tuesday (broken).
  - Confirmed a single date and ESPN's week-based query (`week=N&seasontype=N&year=YYYY`) both still work fine — this is a broken query SHAPE, not the end of usable access to ESPN's API.
  - Fix: `fetchScoreboard()` now fetches every real week of the season in parallel via the week-based query (NFL: preseason/regular/postseason weeks 1-4/1-18/1-5; CFB: regular weeks 1-15, postseason is a single "week 1" holding every bowl/playoff game — all confirmed directly against real ESPN responses, each with a small safety margin since ESPN returns an empty list rather than an error for a week past the real end) and de-dupes the combined result. `daysBack`/`daysForward` params are gone — every call site already wanted the whole season anyway, so the function just always returns everything now, simplifying 3 call sites (`js/season-data.js`, `js/picks.js`) that no longer need to specify a window.
  - Verified via Playwright (full regression suite: week-renumbering, Big Action, League Snapshot, Awards, locked-results, change-pick-autosave, live-vs-final stage gate, pick-locking — all passing) and directly against real ESPN data (full season, ~330 NFL + ~310 CFB games, in ~1.3 seconds).
  - No data was lost and nothing needs manual recovery: player picks live in Supabase (untouched by this), and every other page recomputes everything fresh from scratch on load — the moment this ships, the next page load re-fetches the complete season and regrades everything, fully caught up automatically.
  - Bumped service worker cache to `full-regalia-shell-v119`.

- **SHIPPED (2026-09-14): The whole site now waits for a week's last game to actually FINISH before advancing, not just kick off.** Neil: Home's Big Action had jumped to Week 3 while Monday Night Football (Week 2's last game) was still being played, and the Picks tab followed — a player could no longer see or reference their own Week 2 picks without manually switching back, even though the game everyone actually cared about (6+ picks riding on it) was still live. Awards was correctly still showing Week 2 as active, which Neil confirmed is the right behavior — the ask was to bring the rest of the site in line with that same logic.
  - Root cause: `buildRegaliaWeeks()`'s "is this week still current" check (`js/pick-utils.js`) was based on "does this week have a game still in `pre` (not-yet-kicked-off) status." The instant MNF's status flipped from `pre` to `in` (live), that went false, and Picks/Home's Big Action/the freshness text all advanced to Week 3 — hours before the game actually ended. Awards was never affected since it already keys off *graded* (finished) results, which only exist once a game is final.
  - Discussed the fix with Neil before implementing: only change the "move to next week" stage gate to wait for the last game to be FINAL, not just started. Individual pick locking at kickoff (`js/picks.js`'s own `isLocked` check) is separate and unchanged — you still can't edit a pick once its game starts.
  - Fix: added `hasUnconcludedGame` (broader than the existing `hasPickable` — true for both not-yet-started AND currently-live games, false only once every game in the week is marked `completed`) and switched the "current week" filter to use it instead. A paired Regalia Week stays current as long as EITHER sport's leg has an unconcluded game.
  - Verified via Playwright (two scenarios: MNF live vs. MNF final) and against real live production data at 10 PM ET with the real MNF game still in progress — the site correctly still identifies Week 2 as current; before this fix it would have already flipped to Week 3 hours earlier, right after kickoff. Re-ran the full regression suite (week-renumbering, Big Action, League Snapshot, Awards, locked-results, change-pick-autosave) — zero regressions.
  - Bumped service worker cache to `full-regalia-shell-v118`.
...
- **SHIPPED (2026-09-13): Big Dawg now requires the underdog bet to actually hit.** Neil: "what is the big dawg award tracking? shouldnt you have to get that bet correct in order to win it?" Checked the actual logic in `computeAwardsForWeek()` (`js/awards.js`) — confirmed he was right: `biggestDog` only ever filtered on `pick.line > 0` (took an underdog), with no check on the pick's result at all. Someone could take a wild +24.5 dog that got blown out and still win Big Dawg over someone who took a smaller dog and was actually right.
  ...
  - Since Awards recomputes every week fresh from raw pick data on every page load (nothing is snapshotted), this takes effect retroactively across the whole season's history the moment it ships — not just new weeks going forward.
  - Verified via Playwright: a player who took a bigger underdog that missed no longer beats a player who took a smaller underdog that hit; re-ran the full Awards regression suite to confirm no other award's logic was affected.
  - Bumped service worker cache to `full-regalia-shell-v116`.

- **SHIPPED (2026-09-13): Locked/graded picks now show their result even after every game in that sport's week has kicked off.** Neil: "can we still simply show the results of the bets for NCAA results? Its showing 3/4 for example for Phil, but each user should be able to see the game they picked, if it hit or miss, and the game results." Root cause: `categoriesHtmlForSport()` (`js/picks.js`) bailed out to a blanket "No games with odds available for this week yet" message the moment there were zero still-*pickable* games for that sport's current week — which happens routinely for NCAA (its Thu-Sat slate finishes days before NFL's Thu-Mon one finishes). That blanket bail-out ran BEFORE the function ever reached its per-category logic that already correctly renders a locked pick's real game card, hit/miss badge, and bet summary — so real, fully-computed results were being hidden behind a generic empty state.
  - Fix: the early bail-out now only fires when there's truly nothing to show — no pickable games left AND no already-made pick to show a result for either. A locked pick's category still renders its full result (🔒 pick label, Hit/Miss/Push badge, final score, bet summary) regardless of whether any games remain pickable that week; still-empty categories fall through to their existing normal "Tap to pick a game" placeholder, unaffected.
  - Found and fixed a second, related display bug while verifying this: the "MY PICKS THIS WEEK" progress line was showing a raw internal key ("NCAA · cfb-w2-2") instead of "NCAA · Week 2" for the same reason — `weekBucketLabel()` was only searching the pickable-only games list for a match, which comes up empty once a week's games have all been played. Now searches the full any-status game list instead (safe now that `weekBucketKey()` includes sport, see the earlier fix this same day).
  - Verified via Playwright: a player with a locked, graded NCAA pick in a week where zero NCAA games remain pickable now sees the full result (matchup, Hit badge, 24-10 final score, bet count), while still-unpicked categories show their normal placeholder.
  - Bumped service worker cache to `full-regalia-shell-v115`.

- **SHIPPED (2026-09-13): NFL game cards use "@" for the home team instead of a house emoji, matching NCAA.** Neil: "remove the home emoji from each NFL game -- replace with @ like we did for NCAA games." `gameCardTeamRow()` (`js/live-scores.js`) already put a leading "@" on NCAA's home team; NFL used a 🏠 emoji next to the mascot name instead. Swapped NFL to the same "@ " prefix, removed the now-unused `.game-card-home-icon` CSS. One consistent home-team convention across both sports instead of two different markers.
  - Verified via Playwright: NFL card renders "KC @ Chiefs" with zero house emojis anywhere on the page.
  - Bumped service worker cache to `full-regalia-shell-v114`.

- **SHIPPED (2026-09-08): Fixed a THIRD, deeper bug — NFL/CFB week-key collisions — that was still leaving Big Action stuck on old games.** Neil: "so when exactly will the Big Action games be updated? im still seeing week 1 games?" — the previous fix (scoping Big Action to `currentRegaliaWeek`) turned out incomplete. Root-caused by replaying real production Supabase picks + real ESPN data through the actual deployed logic: `weekBucketKey()`/`weekBucketKeyFromSnapshot()` (`js/pick-utils.js`) never included the SPORT in their key string — just `w{seasonType}-{week}`. NFL's real Week 1 and CFB's real (already-finished) Week 1 both produce the literal same key, so Big Action's "current week" game filter matched CFB's dead Week 1 games right alongside NFL's actually-current Week 1, and CFB's higher accumulated bet count won the ranking every time.
  - This exact collision had been hit and worked around once before — `history.html` has a comment from an earlier bug hunt describing the identical NFL/NCAA-Week-1 collision, worked around there by grouping on a human-readable label instead of the key, rather than fixed at the source. Every OTHER caller (Picks, Awards, the Analytics league filter) happened to always compare within one already-known sport, so the collision stayed invisible until Big Action's new cross-sport "does this game belong to either leg of the current paired week" check hit it directly.
  - Fix: both key functions now prefix the sport (`nfl-w2-1` vs `cfb-w2-1`), eliminating the collision universally — every caller gets the fix for free, no per-caller changes needed (confirmed every existing call site was already comparing within one sport anyway, so this only ever fixes behavior, never changes anything that was already correct).
  - Re-verified against real production data: Big Action's candidate pool dropped from 217 games (CFB Week 1 + the real current week both included) to the correct 102 (just the real current week), and the actual top-3 ranked games are now genuinely NFL Week 1 / CFB Week 2 games with real bets on them, not leftover finished CFB Week 1 games.
  - Re-ran the full regression suite (week-renumbering, Big Action, League Snapshot, Awards, Final badge) — zero regressions, since this only ever removes a false-positive collision, never a legitimate match.
  - Bumped service worker cache to `full-regalia-shell-v113`.

- **SHIPPED (2026-09-08): Fixed a SECOND week-numbering bug the previous fix exposed (real "Week 5" report) + Big Action now scoped to the current week.** Neil: "why does it say week 5 with sept 9-14 dates in the standings table?" — confirmed against real production data (replayed the actual ESPN fetch through the actual app logic before touching code): NFL's 4 preseason weeks (Aug 6-29, seasonType 1, all long finished) were never actually excluded from `buildRegaliaWeeks()` itself — two callers (`currentRegaliaWeek`, used by Home/Standings' freshness text, and the Picks page's own week-numbering fetch) passed them straight through. That was invisible under the OLD numbering order (filter-before-number silently dropped them since they have zero pickable games), but the SAME DAY's earlier fix (numbering the full season first, see below) made those 4 finished preseason weeks permanently claim Regalia Week numbers 1-4 — pushing the real current week (Sep 9-14 games) to a mislabeled "👑 Week 5".
  - Fix: NFL preseason exclusion now lives INSIDE `buildRegaliaWeeks()` itself (`js/pick-utils.js`), not left to each caller to remember — every caller gets it for free regardless of what they pass in. Re-verified against real production data: current week now correctly shows "👑 Week 2" (CFB's already-finished Week 1 season-openers, Aug 29-Sep 7, correctly excluded from the pickable list; NFL Week 1 + CFB Week 2, Sep 9-14, correctly identified as current).
  - Same message, second ask: "want to also ensure that the Big Action games update per week?" — real gap: `mostActiveGames()` (Home's "Big Action" card) ranked candidate games by raw bet count across the ENTIRE season, not just the current week. Once a week ends, its final accumulated bet total sticks around and can easily outrank a brand-new week's just-started picks, so the card would keep showing last week's games indefinitely instead of rotating forward. Fixed by scoping the candidate game pool to the current Regalia Week's games (both sports) before ranking — a new week now always starts fresh from its own games, no matter how many bets an old week racked up.
  - Verified via Playwright: re-ran the real-production-data reproduction (confirms "👑 Week 2," not "Week 5"); a scenario with a heavily-bet finished week (5 bets) vs a lightly-bet current week (1 bet) confirms Big Action now shows the current week's game, not the old one; re-ran the Awards, League Snapshot, and Final-badge test suites (all share the same underlying week logic) to confirm zero regressions.
  - Bumped service worker cache to `full-regalia-shell-v112`.

- **SHIPPED (2026-09-08): Fixed a real week-mislabeling bug + added a "Final" badge once a week officially closes.** Neil: "the week number of ncaa and nfl appears to have moved forward in time. but it still has the week 1 label" — root-caused to `buildRegaliaWeeks()` (`js/pick-utils.js`): its "Crown Week" number was assigned by POSITION in an array that had already been filtered down to "still has a pickable game." The instant Week 1's last game kicked off (dropping Week 1 out of that filtered list), Week 2 slid into array index 0 and INHERITED number 1 — every page showing "the current week" (Home's CTA/freshness line, Standings' freshness line, the Picks page's own week header) relabeled the real Week 2 (correct dates) as "👑 Week 1" (stale, wrong number). Confirmed via a direct reproduction with synthetic data mirroring the real situation (Week 1 fully finished, Week 2 fully upcoming) before touching any code.
  - Fix: `regaliaWeekNumber` is now assigned to the FULL chronological sequence of weeks (finished or not) FIRST, and the "still pickable" filter (the default mode every page except Awards uses) is applied LAST, purely to decide what's shown — never to renumber what's left. A week's number is now a permanent identity once assigned; it can never shift just because an earlier week finished.
  - Bonus fix that fell out of the same restructuring: a paired Regalia Week (one NFL week + its linked NCAA week) now stays visible as long as EITHER sport's leg still has an unplayed game, instead of disappearing the moment just one leg finished (even if the other hadn't even kicked off yet).
  - Second part of the same ask: "can we officially close out the awards (say final) for the first day after a week closes?" — added `isRegaliaWeekFinal(week)` (`js/pick-utils.js`): true starting the CALENDAR DAY after a week's last game, not the instant it ends (Neil wanted a full grace day, since a week's result could still be settling the same day its last game finishes). Home's Awards card now shows a small neutral "Final" badge next to "Week N Awards" once that's true — same `.badge` pill language already used for "Current"/"Next" on the Picks page's week picker, just a new muted `.badge.final` variant since being final isn't an alert state.
  - Verified via Playwright: a direct reproduction test confirming the exact renumbering bug before the fix and its absence after; an end-to-end test across Home/Standings/Picks confirming all three now show "Week 2" (not stale "Week 1") once Week 1 fully finishes; a two-scenario test confirming the Final badge is absent the same day a week's last game finishes and appears the next calendar day; re-ran the existing Awards and League Snapshot test suites (both depend on the same shared week logic) to confirm zero regressions.
  - Bumped service worker cache to `full-regalia-shell-v111`.

- **SHIPPED (2026-09-06): Analytics tab now lands on a league-wide "League Snapshot" dashboard instead of opening straight into two collapsed sections.** Neil wanted the landing view to show total bets placed, overall accuracy, accuracy by NFL vs NCAA, a breakdown by the 4 betting categories, and the single most-bet-on team — filterable by "league week." That's the same unified NFL+NCAA week numbering already used everywhere else (Picks/Standings/Home/Awards), which the app internally calls a **Regalia Week** — so the new filter (a compact `<select>`, "All Weeks" + one entry per week) shows the exact same week numbers Neil already sees on every other page.
  - Per Neil's explicit follow-up after the first pass of this plan ("make sure the landing page is crisp, absolutely no blank space... should look sleek and professional"): this is ONE dense card with hairline-separated internal sections (bets/accuracy numbers, NFL/NCAA stat rows, 4 category KPI tiles, most-bet-team line), not four separately-padded cards — every section's border/spacing collapses away entirely when there's no data to show (e.g. a week with zero picks yet), so there's never a trailing gap.
  - "Total bets placed" and "most bet team" count pending (not-yet-graded) picks too — those are still real locked-in wagers — while accuracy numbers only ever count graded results. A total/Over-Under pick counts toward BOTH teams in that game for "most bet," same convention `teamPickHistoryHtml` already uses for "who's picked X."
  - Under the hood: promoted the private week-bucketing helper that used to live only inside `computeAllWeeklyAwards` (`js/awards.js`) into a shared `attachRegaliaWeeks()` (`js/pick-utils.js`) — Awards and the new Analytics snapshot now use the exact same week-matching logic instead of two copies that could drift apart. Also generalized `computePlayerCategoryRecord` into a new shared `computeCategoryRecord()` (league-wide, no player filter) and promoted `categoryKpiHtml()` (the 4-tile KPI row from last ship's player-detail page) into `js/pick-utils.js` so both pages render category breakdowns identically.
  - Verified via Playwright: a 2-week mocked scenario with graded + pending picks across both sports (14/14 checks — "All Weeks" aggregates correctly, switching weeks recomputes every section to that week's subset, a week with nothing graded yet shows clean empty states not NaN/0%, pending picks count toward volume but not accuracy) plus a re-run of the existing Awards test suite to confirm the shared week-matching refactor didn't change any award results.
  - Bumped service worker cache to `full-regalia-shell-v110`.

- **SHIPPED (2026-09-06): Player detail page now breaks down each player's picks by category, conference, and team — plus a slimmer Player Comparison table on Analytics.** Neil liked the Analytics "Player Comparison" table (overall cover % per player per category) but it was causing horizontal scrolling on phones — at ~330-370px wide (avatar + name + 4 percentage-pill columns) it landed right at the edge of a 375px screen. Dropped the avatar from that table and tightened cell padding/pill width; now renders at ~341px with room to spare, no scroll, name+pills still fully legible.
  - Also added the bigger ask: tapping into a player (`player.html`) now shows three new sections below the existing "Points by week" chart — **By category** (4 KPI tiles: Minus/Plus/Over/Under win% + record, same visual language as the existing "Season summary" tiles), **By conference**, and **By team** (horizontal bar lists, one row per conference/team the player has picked against, sorted by pick count so real sample sizes surface before one-off picks). Conference/team breakdowns are scoped to spread picks only (a total pick belongs to the game, not one team — same reasoning `js/team-stats.js` already uses for why a team's own Over/Under is reported as a split, not a per-team stat); moneyline picks are likewise excluded, consistent with how `pickCategory()` has always treated them everywhere else in the app.
  - Team list starts capped at 8 (most-picked teams first) with a tap-to-expand "Show N more" toggle for the rest, same pattern as Awards history / the "Who's picked" list elsewhere.
  - `computePlayerCategoryRecord()` (previously only inline in `analytics.html`) moved to the shared `js/pick-utils.js` so both pages use the exact same category math.
  - New shared horizontal-bar CSS (`.hbar-row`/`.hbar-track`/`.hbar-fill`) and 4-tile KPI CSS (`.player-kpi-row`/`.player-kpi-tile`) added to `css/style.css`.
  - Verified via Playwright: mocked multi-conference/multi-team scenario (10/10 checks), the >8-teams show-more toggle (7/7 checks), and the Player Comparison width fix (4/4 checks, 341px content in a 375px viewport). Also spot-checked the real conference/team resolution logic against actual production pick data (real team abbreviations across both sports resolved to the correct conference or a legitimate "Other").
  - Bumped service worker cache to `full-regalia-shell-v109`.
...
- **SHIPPED (2026-09-05): Fixed the blank space between Standings and Awards on Home.** Neil noticed a gap below the Standings card in the split-screen row and asked if the "Enter Picks" button could be "more dynamic" to fill it. Root cause: `.home-split-row` had `align-items: start` (added specifically to stop the two cards being force-stretched to equal height, back when nothing filled the shorter one) — but Awards is usually taller than Standings' fixed 3 rows, so the shorter Standings card just left a visible blank gap below it instead.
  ...
  - Bumped service worker cache to `full-regalia-shell-v97`.

- **SHIPPED (2026-09-05): FIXED — Analytics "No data" for finished games + Games tab missing odds on live/final cards + bet-metrics summary.** Neil asked why Analytics showed "No data" for teams with completed games. Root cause, confirmed directly against real ESPN data: **the live scoreboard endpoint drops the `odds` field entirely once a game is no longer "pre"** (78 of 124 real upcoming CFB games had odds; 0 of 53 live/finished ones did). Player pick grading was never affected (each pick stores its own line at pick time), but `computeTeamRecord()` (team ATS/O-U stats) and the Games tab's odds row both read live-fetched `g.odds`, so neither has ever been able to show a line once a game went live or ended.
  - **Fix**: new `backfillMissingOdds(games)` (`js/live-scores.js`) fetches ESPN's separate per-game summary endpoint (`.../summary?event={id}`), which DOES retain the closing line after a game ends — confirmed directly. Its odds shape needed no new parsing: the existing `deriveSpread()` fallback inside `normalizeOdds()` already handles it. Session-cached (`Map<gameId, odds|null>`) so a game is never re-fetched once seen, and mutates games in place so any subset of the shared list can be backfilled without reconciling separate arrays.
  - `live.html`: `render()` backfills the currently-visible games before drawing the list — a live/final game's spread/O-U line shows again. Not applied to Home's compact game cards, which already hide the odds row entirely.
  - `analytics.html`: `renderTeamDetail()` backfills just the viewed team's (and, for a matchup, both teams') finished games before computing stats — scoped per-team, not the whole season up front.
  - **Bet-metrics summary**: `teamPickHistoryHtml()` now shows a quick-glance rollup above the existing per-bet list — total bet count, hit/miss/push record, and a category breakdown (e.g. "1 Minus Spread · 1 Over") — Neil: "how many times, did they win, what category."
  - Verified via Playwright (9/9 on mocked odds-less-then-backfilled data, matching the real bug pattern exactly) plus a re-run against real current ESPN data: backfilled spread/O-U renders correctly on both pages, caching confirmed (summary endpoint not re-fetched on a second render), and a team's legitimately-empty category (never an underdog) still correctly shows "No data yet" rather than being masked.
  - Bumped service worker cache to `full-regalia-shell-v96`.

- **SHIPPED (2026-09-05): Weekly Awards now have season-long memory — career counts + tap-to-expand history, clearer visual hierarchy.** Neil: "if someone is getting ice cold for the 4th time, the league would know it's their fourth time (x4)," plus tapping an award should show who's won it across the season, plus the award name and winner's name were too visually similar to tell apart at a glance.
  - `computeWeeklyAwards` → `computeAllWeeklyAwards` (`js/awards.js`): now computes results for EVERY Regalia Week with graded picks, not just the latest — the per-week computation logic was pulled out into a shared `computeAwardsForWeek()` helper so this didn't duplicate the stats/tie logic.
  - New `computeAwardCareerCounts()`: tallies how many times each player has won each award across the whole season so far — every tied winner in a week counts toward their own tally (confirmed with Neil), not just one arbitrary pick.
  - Current week's winner name(s) now show "(xN)" once they've won that award 2+ times this season (a first-time win stays clean, same minimal-badge philosophy as the streak flame only appearing at 3+ hits).
  - Each award is now tappable — expands a week-by-week history list of every past week that award had a winner, collapsing again on a second tap (same interaction pattern as the Games tab's bet-activity toggles).
  - **Visual hierarchy fix**: award name is now a small muted uppercase caption (not cursive — reads as a category tag), the winner's name is the site's cursive Pacifico brand font in accent color (same treatment as the Picks page's "{Name}'s Picks" heading) so it reads as the actual answer/hero of the row, detail text stays small and muted below.
  - Awards card title now reads "🏅 Week N Awards" instead of a plain "🏅 Awards", so it's immediately clear which week is being shown.
  - Verified via Playwright (11/11): correct current-week winner, "(x2)" showing only on an actual repeat winner (not a first-timer), history expand/collapse showing the right past weeks (excluding the current one, since that's already shown live), and the label/winner font distinction confirmed via computed styles.
  - Bumped service worker cache to `full-regalia-shell-v95`.
