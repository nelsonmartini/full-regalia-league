...
| ~~Data backend~~ **SUPERSEDED 2026-08-01** | ~~Keep the Excel/Google Sheet as the system of record, published as a CSV feed the site reads.~~ **Reversed:** you clarified the actual goal is to *eliminate* the spreadsheet, not just display it online. The site itself becomes the system of record via a real backend (Firebase or Supabase). See `BACKLOG.md` — the specific service choice is deliberately on hold pending a conversation with the site's eventual owner. | Displaying a live-read of the Sheet still would have meant you kept manually entering picks/results into Excel every week — that doesn't kill the manual grind, it just makes the output prettier. A real backend is the only path that actually removes Excel from the loop. |
...
| Live odds | **ESPN's built-in odds data** (embedded free in the same scoreboard response — confirmed CORS-open, no key). **The Odds API is no longer needed** — ESPN alone covers both scores and odds (NFL ~100%, college ~50% coverage). | Verified 2026-08-01 by inspecting the actual API response — avoided a second account/key entirely. |
